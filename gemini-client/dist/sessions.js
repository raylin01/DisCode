...
exports.getGeminiChatsPath = exports.deleteGeminiSession = exports.resolveGeminiSession = exports.listGeminiSessions = void 0;
...
function toGlobalGeminiDir(options) {
    ...
    return (0, path_1.join)(options.homeDir || (0, os_1.homedir)(), GEMINI_DIR_NAME);
}
function toProjectHash(projectRoot) {
    return (0, crypto_1.createHash)('sha256').update((0, path_1.resolve)(projectRoot)).digest('hex');
}
function toChatsPath(options) {
    const geminiDir = toGlobalGeminiDir(options);
    const projectHash = toProjectHash(options.projectRoot);
    return (0, path_1.join)(geminiDir, 'tmp', projectHash, CHATS_DIR);
}
...
async function listGeminiSessions(options) {
    ...
    if (!(0, fs_1.existsSync)(chatsPath))
        return [];
    const files = (await (0, promises_1.readdir)(chatsPath))
    ...
    const parsed = await Promise.all(files.map(async (fileName) => {
        try {
            const filePath = (0, path_1.join)(chatsPath, fileName);
...
exports.listGeminiSessions = listGeminiSessions;
async function resolveGeminiSession(identifier, options) {
    ...
    const chatsPath = toChatsPath(options);
    const filePath = (0, path_1.join)(chatsPath, selected.fileName);
    const raw = await (0, promises_1.readFile)(filePath, 'utf8');
    const record = parseSessionRecord(raw);
    if (!record) {
...
exports.resolveGeminiSession = resolveGeminiSession;
async function deleteGeminiSession(identifier, options) {
    const resolved = await resolveGeminiSession(identifier, options);
    await (0, promises_1.unlink)(resolved.filePath);
    return resolved.session;
}
exports.deleteGeminiSession = deleteGeminiSession;
function getGeminiChatsPath(options) {
    return toChatsPath(options);
}
exports.getGeminiChatsPath = getGeminiChatsPath;
