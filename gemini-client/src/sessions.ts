...
import type {
  GeminiSessionInfo,
  GeminiSessionLocatorOptions,
  GeminiSessionMessage,
  GeminiSessionRecord,
  ResolvedGeminiSession
} from './types.js';
...
function toGlobalGeminiDir(options: Pick<GeminiSessionLocatorOptions, 'homeDir' | 'geminiDir'>): string {
  if (options.geminiDir) return options.geminiDir;
  return join(options.homeDir || homedir(), GEMINI_DIR_NAME);
}

function toProjectHash(projectRoot: string): string {
  return createHash('sha256').update(resolve(projectRoot)).digest('hex');
}

function toChatsPath(options: GeminiSessionLocatorOptions): string {
...
function hasUserOrAssistantMessage(messages: GeminiSessionMessage[]): boolean {
  return messages.some((msg) => msg.type === 'user' || msg.type === 'gemini');
}

function extractFirstUserMessage(messages: GeminiSessionMessage[]): string {
...
function toDisplayName(summary: string | undefined, fallback: string): string {
  const candidate = cleanMessage(summary || '');
  if (candidate.length > 0) return candidate;
  return fallback;
}

function parseSessionRecord(raw: string): GeminiSessionRecord | null {
  try {
    const parsed: unknown = JSON.parse(raw);
    if (!isRecord(parsed)) return null;
    if (typeof parsed.sessionId !== 'string') return null;
    if (typeof parsed.projectHash !== 'string') return null;
...
export async function listGeminiSessions(options: GeminiSessionLocatorOptions): Promise<GeminiSessionInfo[]> {
  const chatsPath = toChatsPath(options);
  if (!existsSync(chatsPath)) return [];
  ...
  const parsed = await Promise.all(files.map(async (fileName) => {
    try {
      ...
      if (!hasUserOrAssistantMessage(record.messages)) return null;
      ...
      const session: GeminiSessionInfo = {
        id: record.sessionId,
  ...
  const uniqueById = new Map<string, GeminiSessionInfo>();
  ...
  const sessions = Array.from(uniqueById.values()).sort(
...
export async function resolveGeminiSession(
  identifier: string,
  options: GeminiSessionLocatorOptions
): Promise<ResolvedGeminiSession> {
  ...
  let selected: GeminiSessionInfo | undefined;
  ...
  if (!selected) {
    throw new Error(
      `Invalid session identifier "${identifier}". Use "latest", an index number, or a session UUID.`
    );
  }
...
export function getGeminiChatsPath(options: GeminiSessionLocatorOptions): string {
