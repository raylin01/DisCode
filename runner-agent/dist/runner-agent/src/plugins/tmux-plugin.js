...
// TmuxSession
// ============================================================================
class TmuxSession extends EventEmitter {
    sessionId;
    config;
    createdAt;
    status = 'idle';
    ...
    constructor(config, tmuxSession, tmuxPath, isOwned = true) {
        super();
    ...
    async sendApproval(optionNumber, _message, _requestId) {
 ...
 */
async function sendToTmuxSafe(tmuxSession, text, tmuxPath) {
 ...
 */
async function capturePane(tmuxSession, lines = 100, tmuxPath) {
    validateTmuxSession(tmuxSession);
    const { stdout } = await execFileAsync(tmuxPath, ['capture-pane', '-t', tmuxSession, '-p', '-S', `-${lines}`], { ...EXEC_OPTIONS, maxBuffer: 1024 * 1024 });
    return stdout;
}
/**
...
function detectPermissionPrompt(output) {
    ...
    if (!hasFooter && !hasSelector && promptType === 'permission')
        return null;
...
function detectShellPrompt(output) {
    ...
    if (lines.length === 0)
        return true; // Empty = probably idle
...
export class TmuxPlugin extends BasePlugin {
    ...
    pollInterval;
    healthInterval;
    tmuxPath = 'tmux'; // Default to just 'tmux', will be updated in initialize
    // Polling for new sessions
    sessionDiscoveryInterval;
    discoveredSessions = new Set();
    skillManager;
    async initialize() {
        ...
        pluginManager.on('hook_event', (event) => {
    ...
    async getSessionCwd(sessionId) {
        try {
            const { stdout } = await execFileAsync(this.tmuxPath, ['display-message', '-t', sessionId, '-p', '#{pane_current_path}'], EXEC_OPTIONS);
        ...
        this.pollInterval = setInterval(() => {
            ...
            for (const session of this.sessions.values()) {
                if (session.status !== 'offline') {
                    ...
                    if (!session.isOwned)
                        this.log(`[DEBUG] Watched session ${session.sessionId} is OFFLINE, skipping poll`);
                }
            }
    ...
    getNewContent(oldOutput, newOutput, isClaudeCode = true) {
        if (!oldOutput)
            return newOutput;
