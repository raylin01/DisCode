 ...
 */
function waitForApproval(requestId, timeout, pendingApprovals) {
    return new Promise((resolve, reject) => {
...
export function createHttpServer(config, deps) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && url.pathname === '/approval') {
            try {
                ...
                const approvalReq = {
                    toolName: (rawData.tool_name || rawData.toolName),
                ...
                try {
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Approval timeout')), config.approvalTimeout));
